...
void make_com_fl_arg(char **mas, t_tree *node)
{
	...
	while(mas[k])
	{
		if(ft_strcmp(mas[k], "\">\"") == 0 ||
		   ft_strcmp(mas[k], "\">>\"") == 0 || ft_strcmp(mas[k], "\"<\"") == 0
		   || ft_strcmp(mas[k], "\"<<\"")== 0 ||
		   ft_strcmp(mas[k], "\'>\'") == 0 ||ft_strcmp(mas[k], "\'>>\'") == 0
		   || ft_strcmp(mas[k], "\'<\'") == 0 ||ft_strcmp(mas[k], "\'<<\'")
		   == 0)
			remove_quotes(&mas[k]);
...
void *make_tree(char **mas)
{
	...
	int i;
 	...
 	else if ( find_str(mas, ">>") || find_str(mas, ">") || find_str(mas, "<<")
 	|| find_str(mas, "<"))
	{
 		i = 0;
 		while(mas[i] && ft_strcmp(mas[i], ">") != 0 && ft_strcmp(mas[i],
																  ">>") != 0
